...
describe CucumberFTC::ECP::EcpClasses do
	before :each do
		@instance = CucumberFTC::ECP::EcpClasses.new
	end

	it 'should allows class add operations' do
		@instance.add_class(mock(CucumberFTC::ECP::EcpClass))
	end
	...
	def test_two_class_ownership opts={}
		...
		{
			mock(
				CucumberFTC::ECP::EcpClass,
				:name=>'class1'
			) => true,
			mock(
				CucumberFTC::ECP::EcpClass,
				:name=>'class2'
			) =>opts[:more_than_one_class_validates]
	...
	it 'should raise an exception if more than one class owns a obj' do
		lambda{
			...
			CucumberFTC::ECP::MoreThanOneClassOwnsAObjectException
		)
	end
end
